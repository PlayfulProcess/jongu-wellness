...
export async function POST(request: NextRequest) {
  try {
    ...
    // Update the tool_data to mark as approved and reviewed
    const updatedToolData = {
      ...tool.tool_data,
      reviewed: 'true',
      is_active: 'true', // Approve the tool
      approved_at: new Date().toISOString(),
      approved_by: 'admin'
    };
    
    // Update the tool with approved status
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to approve tool' }, { status: 500 });
    }
